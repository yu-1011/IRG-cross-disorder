...
eigmat = ad_net$MEs; colnames(eigmat) = gsub("ME","",colnames(eigmat))
kME = signedKME(ad_datExpr,ad_net$MEs); colnames(ad_net$MEs) = gsub("kME", "", colnames( ad_net$MEs))


datProbes = multiExpr[[1]]$datProbes[,-1]


#Write module genes
colors <- aad_net$colors
datExpr <- aad_datExpr
for(c in unique(colors)) {
  mod = colnames(datExpr)[colors==c]
  mod.gene = cbind(datProbes[mod,],c)
  write.table(file = paste("./results/tables/GeneSets/modules/input/aad_", c, ".txt", sep=""), mod.gene, row.names = F, quote=F)
}
write.table(file="./results/tables/GeneSets/modules/background/background.txt", data.frame(Gene=rownames(datExpr), Id="En"), row.names=F, quote=F)



## Calculate GO enrichment of Disease-associated modules
resultsGO = data.frame()
colors <- pd_net$colors
for (m in c("purple")) {
  me_name = paste("ME", m, sep="")
  i = which(m == unique(colors))  
  
  ## GO Enrichment
  query = colnames(datExpr)[colors==m]
  ...
  ck <- enrichGO(query, 
   ...
   #                    readable      = TRUE)
  #dotplot(ck) + 
   # ggplot2::scale_y_discrete(labels=function(x) str_wrap(x, width=55))+
    #ggplot2::scale_size(range=c(2, 6))
  #write.table(as.data.frame(ck),file="./results/tables/UniqueDifferentialGeneAnnotationDirection.txt",sep="\t",row.names = F,col.names = T,quote = F)
...
write.csv(file="./results/tables/GeneSets/gProfiler_resultsasdtanGO.190413.csv", resultsGO)


resultsGOsub2 = data.frame()
for(c in sort(c("cyan","tan","yellow"))) {
  idx = which(resultsGO$m==c) 
  resultsGOsub2 = rbind(resultsGO[idx[5:1],],resultsGOsub2)  
}


pdf("./results/figures/PreManuscript/Manuscript/Fig3E-Module-GO0413.pdf",width = 4,height=3)
...
for(mod in rownames(cell.p.zhang)) {
    f = fisher.iteration(pSI.output, colnames(datExpr)[moduleColors==mod],p.adjust = F)
...
to_plot = cell.p.zhang.fdr[c("yellow", "cyan",  "tan"),]


dendro.col = as.dendrogram(hclust(as.dist(1-bicor(zhang.datExpr)), method="average"))
denro.row= as.dendrogram(hclust(as.dist(1-bicor(eigmat[,unique(moduleColors)])),method="average"))

pdf("./results/figures/PreManuscript/Manuscript/Fig3F-CellType0413.pdf",width=6,height=5)
heatmap.2(-log10(to_plot),col=blueWhiteRed(1000,1)[500:1000],
          scale="none",trace="none",cexRow = 0.8,cexCol =0.8, density.info = "none",
...
write.csv(out,file="./results/tables/Manuscript/TableS2-pd-CellType.csv")
...
write.csv(resultsTFBS,file="./results/tables/Manuscript/TableS2-adTFBS.csv")
...
write.csv(hubGenes,file="./results/tables/TableS2-adHubGeneTFs.csv", row.names = F)
...
table.p = matrix(NA, nrow=length(unique(moduleColors)), ncol=19)
