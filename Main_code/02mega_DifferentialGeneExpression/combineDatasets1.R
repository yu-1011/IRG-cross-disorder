...
legend("topright", (levels(all_datMeta$Study)), col=c(1:8), pch=16, cex=0.5)
...
#Normalize by Study
mod = model.matrix(~Group+Sex+Age, data=all_datMeta)
batch = as.factor(all_datMeta$Study)
datExpr = ComBat(all_datExpr, batch=batch, mod=mod)

##QC - PostCombat
par(mfrow=c(2,2))
plot(density(datExpr[,1]), col = as.numeric(all_datMeta$Study[1]), , xlab="Intensity (log2)", ylab="Density", main="Mega-Analysis: Post-Combat")
...
datMeta = all_datMeta
save(file = "./working_data//NetworkAnalysis///All_datasets_combined_081518.RData", datExpr,datMeta,multiExpr)
