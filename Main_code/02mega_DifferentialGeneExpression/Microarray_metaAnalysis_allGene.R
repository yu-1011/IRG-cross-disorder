...
genes= intersect(rownames(AD_multiExpr[[1]]$datExpr), rownames(AD_multiExpr[[2]]$datExpr))
...
AD_datMeta=data.frame(Study=NA, Subject=NA, Group=NA)
...
AD_datMeta = rbind(AD_datMeta, AD_multiExpr[[i]]$datMeta[,c("Study", "Subject","Group")])
...
for(i in 1:nrow(AD_datExpr)) {
  ...
  tryCatch({
    ad_meta[i,] = summary(lme(expr~ Group + Study,data = AD_datMeta, random=~1|Subject))$tTable[2,c(1,2,5)]
  }, error=function(e){})
}
...
ad_meta$symbol=AD_multiExpr[[1]]$datProbes$external_gene_id[match(genes, AD_multiExpr[[1]]$datProbes$ensembl_gene_id)]
#ad_meta= ad_meta[!apply(is.na(ad_meta),1,any),]
write.csv(file="./results/tables/Microarray_ad_metaanalysis_122718.csv",ad_meta)
...
write.csv(file="./results/tables//Microarray_ASD_metaanalysis_122718.csv",asd_meta)
...
write.csv(file="./results/tables/Microarray_SCZ_metaanalysis_122718.csv",scz_meta)
...
write.csv(file="./results/tables/Microarray_BD_metaanalysis_122718.csv",bd_meta)
...
write.csv(file="./results/tables/Microarray_MDD_metaanalysis_122718.csv",mdd_meta)
...
AAD_datMeta=data.frame(Study=AAD_Study, Subject=AAD_Subject, Group=AAD_Group)
...
write.csv(file="./results/tables/Microarray_AAD_metaanalysis_122718.csv",aad_meta)
...
genes= intersect(intersect(intersect(rownames(PD_multiExpr[[2]]$datExpr), rownames(PD_multiExpr[[2]]$datExpr)), rownames(PD_multiExpr[[3]]$datExpr)), rownames(PD_multiExpr[[4]]$datExpr))
for(i in 3:n) genes = intersect(genes, rownames(PD_multiExpr[[i]]$datExpr))
for(i in 2:n) PD_multiExpr[[i]]$datExpr = PD_multiExpr[[i]]$datExpr[match(genes,rownames(PD_multiExpr[[i]]$datExpr)),]
names(PD_multiExpr[[1]]$datMeta) <- gsub("disease_state","Group",names(PD_multiExpr[[1]]$datMeta))
names(PD_multiExpr[[2]]$datMeta) <- gsub("disease","Group",names(PD_multiExpr[[2]]$datMeta))
names(PD_multiExpr[[3]]$datMeta) <- gsub("disease_state","Group",names(PD_multiExpr[[3]]$datMeta))
names(PD_multiExpr[[4]]$datMeta) <- gsub("DiseaseState","Group",names(PD_multiExpr[[4]]$datMeta))
PD_multiExpr[[3]]$datMeta$Source <- rownames(PD_multiExpr[[3]]$datMeta)

PD_datExpr = data.frame(row.names = genes)
PD_datMeta=data.frame(Study=NA, Subject=NA, Group=NA)
for(i in 2:n) {
  ...
  PD_datMeta = rbind(PD_datMeta, PD_multiExpr[[i]]$datMeta[,c("Study", "Subject","Group")])
...
PD_meta_2 = matrix(NA, nrow=nrow(PD_datExpr), ncol=3)
for(i in 1:nrow(PD_datExpr)) {
  if(i%%100==0) print(i)
  expr = as.numeric(PD_datExpr[i,])
  tryCatch({
    PD_meta_2[i,] = summary(lme(expr~ Group + Study,data = PD_datMeta, random=~1|Subject))$tTable[2,c(1,2,5)]
  }, error=function(e){})
}

PD_meta_2=as.data.frame(PD_meta_2)
colnames(PD_meta_2) = c("beta", "SE", "p")
rownames(PD_meta_2) = genes
PD_meta_2$fdr = p.adjust(PD_meta_2$p, "fdr")
PD_meta_2$symbol=PD_multiExpr[[2]]$datProbes$external_gene_id[match(genes, PD_multiExpr[[2]]$datProbes$ensembl_gene_id)]
#PD_meta= PD_meta[!apply(is.na(PD_meta),1,any),]

write.csv(file="./results/tables/Microarray_PD_2_metaanalysis_122718.csv",PD_meta_2)
...
write.csv(file="./results/tables/Microarray_IBD_metaanalysis_122718.csv",ibd_meta)
...
save(file="./working_data/Microarray/Microarray_compiledForPermutationTesting_123018.RData", multiExpr)
