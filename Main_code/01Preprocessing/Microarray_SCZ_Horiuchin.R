...
#Load & clean metaData
datMeta = read.csv("./raw_data/Microarray/Horiuchi_GSE73129_scz_blood/SCZ_Blood_GSE73129SampleInfo.csv",header = T)
rownames(datMeta) = datMeta$Sample_geo_accession
datMeta$disease = factor(datMeta$Group,levels =  c("CTL", "SCZ"))
...
datExpr = read.delim(file="./raw_data/Microarray/Horiuchi_GSE73129_scz_blood/SCZ_Blood_GSE73129",comment="!")
...
geneDat<-getBM(attributes=c("affy_hg_u133_plus_2", "ensembl_gene_id", "entrezgene", "external_gene_id", "chromosome_name", "start_position", "end_position"), filters="affy_hg_u133a", values=rownames(datExpr),
               mart = ensembl)
...
save(file="./working_data/Microarray/01_Normalized/Microarray_SCZ_Horiuchin_normalized.RData", datMeta, datExpr, datProbes)
pdf("./results/figures/MicroarrayQC/SCZ_Horiuchin_QC.SCZf", width=11,height=8.5)
par(mfrow=c(3,4))
...
save(file="./working_data/Microarray/02_NormalizedBalanced_noBatchCorrection/Microarray_SCZ_Horiuchin_normalized_balanced.RData", datMeta, datExpr, datProbes,model)
...
mod = model.matrix(~disease+Sex+Age, data=datMeta)
...
save(file = "./working_data/Microarray/03_NormalizedBalanced_ComBat_CR_cleaned/Microarray_SCZ_Horiuchin_blood_normalized_CR_cleaned.RData", datExpr, datMeta, datProbes)
