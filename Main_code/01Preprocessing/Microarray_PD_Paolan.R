...
#Load & clean metaData
datMeta = read.csv("./raw_data/Microarray/PD_GSE72267_blood/E-GEOD-72267.sdrf.csv",header = T)
rownames(datMeta) = datMeta$Source.Name
datMeta$disease = factor(datMeta$Group,levels =  c("CTL", "PD"))
...
datExpr = read.delim(file="./raw_data/Microarray/PD_GSE72267_blood/GSE72267_series_matrix.txt",comment="!")
...
getinfo <- c("affy_hg_u133a_2", "ensembl_gene_id", "entrezgene", "external_gene_id", "chromosome_name", "start_position", "end_position")
#geneDat<- getBM(attributes = getinfo,filters=identifier,values=rownames(datExpr),mart=ensembl)
geneDat<-getBM(attributes=c("affy_hg_u133a_2", "ensembl_gene_id", "entrezgene", "external_gene_id", "chromosome_name", "start_position", "end_position"), filters="affy_hg_u133a", values=rownames(datExpr),
               mart = ensembl)

idx = match(rownames(datExpr), geneDat$affy_hg_u133a_2)
datProbes = cbind(rownames(datExpr), geneDat[idx,])

colnames(datExpr) = rownames(datMeta)
save(file="./working_data/Microarray/01_Normalized/Microarray_PD_Paolan_normalized.RData", datMeta, datExpr, datProbes)
pdf("./results/figures/MicroarrayQC/PD_Paolan_QC.pdf", width=11,height=8.5)
par(mfrow=c(3,4))
...
model = model.matrix(~disease, data=datMeta)
save(file="./working_data/Microarray/02_NormalizedBalanced_noBatchCorrection/Microarray_PD_Paolan_normalized_balanced.RData", datMeta, datExpr, datProbes,model)
...
idx = match(CR$group2row[,"selectedRowID"], datProbes$affy_hg_u133a_2)
...
save(file = "./working_data/Microarray/03_NormalizedBalanced_ComBat_CR_cleaned/Microarray_PD_Paolan_blood_normalized_CR_cleaned.RData", datExpr, datMeta, datProbes)
