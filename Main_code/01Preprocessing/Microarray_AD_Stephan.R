...
#Load & clean metaData
datMeta = read.csv("./raw_data/Microarray/AD_Stephan_GSE5281/GSE5281_sample_characteristics.csv")
rownames(datMeta) = datMeta$GEO_Accession
datMeta$Disease_State = factor(datMeta$Disease_State,levels =  c("normal", "Alzheimer's_Disease"))
datMeta$Group = datMeta$Disease_State
datMeta$Group <- gsub("Alzheimer's_Disease","AD",datMeta$Group)
datMeta$Group <- gsub("normal","CTL",datMeta$Group)
datMeta$Group = factor(datMeta$Group,levels =  c("AD", "CTL"))
...
datMeta$Subject = datMeta$Sample_Name
datMeta$Region = as.factor(datMeta$Organ_Region)
datMeta$Cell_Type = as.factor(datMeta$Cell_Type)
datMeta$Study="AD.Stephan"
...
datExpr = read.delim(file="./raw_data/Microarray/AD_Stephan_GSE5281/GSE5281_series_matrix.txt",comment="!")
...
idx = match(colnames(datExpr), datMeta$GEO_Accession)
datMeta = datMeta[idx,]
...
save(file="./working_data/Microarray/01_Normalized/Microarray_AD_Stephan_normalized.RData", datMeta, datExpr, datProbes)
pdf("./results/figures/MicroarrayQC/AD_Stephan_QC.pdf", width=11,height=8.5)
par(mfrow=c(3,4))
...
model = model.matrix(~Disease_State+Age+Sex+Cell_Type+Organ_Region, data=datMeta)
save(file="./working_data/Microarray/02_NormalizedBalanced_noBatchCorrection/Microarray_AD_Stephan_normalized_balanced.RData", datMeta, datExpr, datProbes,model)
...
i = 1
plot(density((datExpr[,i]), na.rm=T), col = as.numeric(datMeta$Disease_State[i]), main="Hist of Log2 Exp", xlab = "log2 exp", xlim=c(3, 16), ylim=c(0,0.2)); 
...
A = anova(lm(as.numeric(datMeta$Disease_State) ~ as.factor(datMeta$Cell_Type))); p = A$"Pr(>F)"[1];   plot(datMeta$Disease_State ~ as.factor(datMeta$Cell_Type), main=paste("Cell_Type, p=", signif(p,2)), ylab="", xlab="", xlim=c(0,1))
...
datMeta = datMeta[which(datMeta$GEO_Accession!="GSM238797"),]
datExpr = datExpr[,rownames(datMeta)]

## Regress all Technical and Non-DX biological covariates
#datMeta$RIN[is.na(datMeta$RIN)]= mean(dat1Meta$RIN,na.rm=T)
X = model.matrix(~Disease_State+Cell_Type+Age+Sex, data=datMeta)
Y = datExpr
beta = (solve(t(X)%*%X)%*%t(X))%*%t(Y)
to_regress = (as.matrix(X[,1:2]) %*% (as.matrix(beta[1:2,]))) 
datExpr = datExpr - t(to_regress)

save(file = "./working_data/Microarray/03_NormalizedBalanced_ComBat_CR_cleaned/Microarray_AD_Stephan_normalized_CR_cleaned.RData", datExpr, datMeta, datProbes)
