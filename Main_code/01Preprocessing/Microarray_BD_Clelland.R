...
#Load & clean metaData
datMeta = read.csv("./raw_data/Microarray/GSE46449_blood_BD/E-GEOD-46449.sdrf.csv",header = T)
rownames(datMeta) = datMeta$Assay.Name
#datMeta$Characteristics.disease. <- gsub("normal","CTL",datMeta$Characteristics.disease.)
#datMeta$Characteristics.disease. <- gsub("Alzheimers disease","BD",datMeta$Characteristics.disease.)
datMeta$disease = factor(datMeta$FactorValue..GENOTYPE.,levels =  c("CTL", "BD"))
...
datExpr = read.delim(file="./raw_data/Microarray/GSE46449_blood_BD/GSE46449_series_matrix.txt",comment="!",sep = "\t")
...
save(file="./working_data/Microarray/01_Normalized/Microarray_BD_Clelland_normalized.RData", datMeta, datExpr, datProbes)
pdf("./results/figures/MicroarrayQC/BD_Clelland_QC.pdf", width=11,height=8.5)
par(mfrow=c(3,4))
...
model = model.matrix(~disease+Age, data=datMeta)
save(file="./working_data/Microarray/02_NormalizedBalanced_noBatchCorrection/Microarray_BD_Clelland_normalized_balanced.RData", datMeta, datExpr, datProbes,model)
...
plotDendroAndColors(tree, colors = cbind(disease_col,age_col), groupLabels = c("disease", "Age"), cex.colorLabels=0.6, cex.dendroLabels=0.5)
...
save(file = "./working_data/Microarray/03_NormalizedBalanced_ComBat_CR_cleaned/Microarray_BD_Clelland_blood_normalized_CR_cleaned.RData", datExpr, datMeta, datProbes)
